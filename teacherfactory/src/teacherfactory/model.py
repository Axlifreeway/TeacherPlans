...
class LessonStep(BaseModel):
    ...
    number: int = Field(
        description="Порядковый номер этапа занятия"
    )
    stage: str = Field(
    ...
    time: str = Field(
        description="Время, отведённое на этап (например: '5 мин', '20 мин')"
    )
    teacher: str = Field(
        description="Деятельность преподавателя на данном этапе"
    )
    student: str = Field(
        description="Деятельность студентов на данном этапе"
    )
    methods: str = Field(
        description="Методы и приёмы обучения, применяемые на этапе"
    )
    result: str = Field(
        description="Планируемый результат этапа"
    )
...
class LessonTasks(BaseModel):
    ...
    educational: str = Field(
        description="Образовательная задача занятия"
    )
    developmental: str = Field(
        description="Развивающая задача занятия"
    )
    upbringing: str = Field(
        description="Воспитательная задача занятия"
    )
    pedagogical: str = Field(
        description="Педагогическая задача (методическая цель преподавателя)"
    )
...
class PlannedOutcomes(BaseModel):
    ...
    knowledge_indicator: str = Field(
        description="Индикатор достижения результата по знаниям "
                    "(что студент должен знать)"
    )
    knowledge_control: str = Field(
        description="Форма контроля знаний "
                    "(например: 'устный опрос', 'тестирование')"
    )
    skill_indicator: str = Field(
        description="Индикатор достижения результата по умениям "
                    "(что студент должен уметь)"
    )
    skill_control: str = Field(
        description="Форма контроля умений "
                    "(например: 'практическая работа', 'решение задач')"
    )
    ...
    ability_control: str = Field(
        description="Форма контроля навыков "
                    "(например: 'лабораторная работа', 'демонстрация')"
    )
...
class Resources(BaseModel):
    ...
    literature: str = Field(
        description="Основная и дополнительная литература"
    )
    internet_resources: str = Field(
        description="Интернет-ресурсы (ЭБС, сайты, порталы)"
    )
    other_materials: str = Field(
        description="Другие материалы (раздатки, оборудование, ПО)"
    )
...
class LessonCard(BaseModel):
    ...
    lesson_number: int = Field(
        description="Порядковый номер занятия"
    )
    date: str = Field(
        description="Дата проведения занятия (формат: 'ДД.ММ.ГГГГ')"
    )
    ...
    discipline: str = Field(
        description="Наименование дисциплины / МДК"
    )
    specialty: str = Field(
    ...
    duration: int = Field(
        description="Продолжительность занятия в минутах (обычно 45 или 90)"
    )
    ...
    # --- Цель и задачи ---
    goal: str = Field(
        description="Цель занятия"
    )
    tasks: LessonTasks = Field(
        description="Задачи занятия"
    )

    # --- Компетенции ---
    competencies_ok: str = Field(
        description="Общие компетенции (ОК) с кодами и формулировками, "
    ...
    outcomes: PlannedOutcomes = Field(
        description="Планируемые результаты (знания, умения, навыки)"
    )

    # --- Ресурсы ---
    resources: Resources = Field(
        description="Учебно-методическое обеспечение"
    )

    # --- Ход занятия ---
    lesson_structure: list[LessonStep] = Field(
        description="Ход занятия — список этапов урока"
    )
    ...
    def to_template_context(self) -> dict:
        ...
        return {
            ...
            "lesson_structure": [
                step.model_dump() for step in self.lesson_structure
            ],
        }
